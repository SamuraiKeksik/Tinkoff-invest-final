...
namespace TinkoffInvestLib
{
    public class TinkoffInvestSandboxBot
    {
        ...
        private TinkoffInvestSandboxBot(string token)
        {
            var client = InvestApiClientFactory.Create(token, true);
        ...
        public static async Task<TinkoffInvestSandboxBot> CreateTinkoffInvestBotAsync(string token) //Создает объект бота и выполняет асинхронный метод
        {            
            var bot = new TinkoffInvestSandboxBot(token);
        ...
        public async Task<bool> SellAllInstruments(Account account)
        {
